...
void reiniciar(int a,int b, int f1, int f2, int anch, int alt, char m, char n, char p,int x,int y,teleporter t1,teleporter t2,bloqueo b1[],int T1,int T2,int B1[]);
//Rehacemos el laberinto completamente con la cruceta en el inicio de la semilla.
void construir(int a,int b, int f1, int f2, int anch, int alt, char m, char n, char p,int x,int y,teleporter t1,teleporter t2,bloqueo b1[]);
...
int main()
{
     ...
     bloqueo b1[4]; // declaramos el vector de bloqueos;
       int i, j, anch, alt, rad1, rad2,f1=0,f2=0,esp=0,x,y,a,b,lapso=200,T1,T2,B1[bloc];
...
while(1)
{
    ...
    if((lab[b+f2][a+1+f1]==m)||(lab[b+f2][a+1+f1]==t1.identificador)||(lab[b+f2][a+1+f1]==t2.identificador)||(lab[b+f2][a+1+f1]=='B'))//Si el siquiente espacio a la derecha es un espacio
         ...
         if(GetAsyncKeyState(0x27))//Si la flecha derecha es presionada
{
    ...
    system("cls");
    construir(a,b,f1,f2,anch,alt,m,n,p,x,y,t1,t2,b1);
}
}
if((lab[b+f2][f1+a-1]==m)||(lab[b+f2][f1+a-1]==t1.identificador)||(lab[b+f2][f1+a-1]==t2.identificador)||(lab[b+f2][f1+a-1]=='B')) //Si el espacio a la izquierda es un espacio
{//Si detecta un inicio o fin de teleportador pasa;
...
if((lab[f2][a+f1]==m)||(lab[f2][a+f1]==t1.identificador)||(lab[f2][a+f1]==t2.identificador)||(lab[f2][a+f1]=='B')) //Si el elemento de abajo es un espacio
{//Si detecta un inicio o fin de teleportador pasa;
...
if((lab[b+1+f2][a+f1]==m)||(lab[b+1+f2][a+f1]==t1.identificador)||(lab[b+1+f2][a+f1]==t2.identificador)||(lab[b+1+f2][a+f1]=='B'))//Si el espacio superior es un espacio
{//Si detecta un inicio o fin de teleportador pasa;
...
if(GetAsyncKeyState(0x09))//Si se presiona el tabulador;
{
    ...
    b1[2].identificador='B';
    b1[3].identificador='B';
reiniciar(a,b,f1,f2,anch,alt,m,n,p,x,y,t1,t2,b1,T1,T2,B1);//Iniciar reinicio;
}
